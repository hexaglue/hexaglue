...
/**
 * Builds {@link ValueObject} instances from classification results.
 *
 * <p>This builder transforms a classified {@link TypeNode} into a {@link ValueObject}
 * by constructing the type structure and classification trace.</p>
 *
 * <h2>Usage</h2>
 * <pre>{@code
 * ValueObjectBuilder builder = new ValueObjectBuilder(structureBuilder, traceConverter);
 * ValueObject valueObject = builder.build(typeNode, classification, context);
 * }</pre>
 *
 * @since 4.1.0
 */
public final class ValueObjectBuilder {

    private final TypeStructureBuilder structureBuilder;
    private final ClassificationTraceConverter traceConverter;

    /**
     * Creates a new ValueObjectBuilder.
     *
     ...
     */
    public ValueObjectBuilder(TypeStructureBuilder structureBuilder, ClassificationTraceConverter traceConverter) {
    ...
    /**
     * Builds a ValueObject from a classified type node.
     *
     * @param typeNode the type node to build from
     * @param classification the classification result
     * @param context the builder context
     * @return the built ValueObject
     * @throws NullPointerException if any argument is null
     */
    public ValueObject build(TypeNode typeNode, ClassificationResult classification, BuilderContext context) {
        ...
        return ValueObject.of(id, structure, trace);
    }
}
