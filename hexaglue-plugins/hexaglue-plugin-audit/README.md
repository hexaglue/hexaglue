...
| `errorOnBlocker` | boolean |  | Treat BLOCKER violations as audit errors. Overrides the YAML errorOnBlocker setting |
| `errorOnCritical` | boolean |  | Treat CRITICAL violations as audit errors. Overrides the YAML errorOnCritical setting |
| `failOnError` | boolean |  | Fail the Maven build when audit errors are found. Overrides the YAML failOnError setting |
| `failOnUnclassified` | boolean | `false` | Whether to fail the build if unclassified types remain |
| `reportDirectory` | string | `${project.build.directory}/hexaglue/reports` | Output directory for audit reports. Reports are generated by the audit plugin (DddAuditPlugin) in the audit/ subdirectory |
| `skip` | boolean | `false` | Skip HexaGlue audit execution |
...
| `validationReportPath` | string | `${project.build.directory}/hexaglue/reports/validation/validation-report.md` | Output path for the validation report file. If specified, a Markdown report will be written to this location in addition to the console output |
<!-- GENERATED:MAVEN:END -->
...
|--------|---------|-------------|
| `failOnError` | `true` | Fail build when audit errors are found |
...
- Java 17 or higher
- HexaGlue 6.1.1-SNAPSHOT or higher
