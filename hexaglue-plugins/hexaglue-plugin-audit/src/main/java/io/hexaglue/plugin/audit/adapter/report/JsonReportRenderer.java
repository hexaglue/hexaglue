...
public class JsonReportRenderer implements ReportRenderer {
    ...
    private void appendMetadata(StringBuilder json, ReportMetadata metadata) {
        ...
        json.append("    \"projectName\": ").append(quote(metadata.projectName())).append(",\n");
        json.append("    \"projectVersion\": ").append(quote(metadata.projectVersion())).append(",\n");
        json.append("    \"timestamp\": ").append(quote(ISO_FORMATTER.format(metadata.timestamp()))).append(",\n");
        json.append("    \"analysisDuration\": ").append(quote(metadata.duration())).append(",\n");
        json.append("    \"hexaglueVersion\": ").append(quote(metadata.hexaglueVersion())).append(",\n");
        json.append("    \"pluginVersion\": ").append(quote(metadata.pluginVersion())).append("\n");
        json.append("  }");
    }
    ...
    private void appendVerdict(StringBuilder json, Verdict verdict) {
        ...
        json.append("    \"statusReason\": ").append(quote(verdict.statusReason())).append(",\n");
        ...
        json.append("      \"required\": ").append(verdict.immediateAction().required()).append(",\n");
        json.append("      \"message\": ").append(quote(verdict.immediateAction().message())).append(",\n");
        json.append("      \"reference\": ").append(quote(verdict.immediateAction().reference())).append("\n");
        json.append("    }\n");
        json.append("  }");
    }
    ...
    private void appendKpis(StringBuilder json, List<KPI> kpis) {
        ...
        for (int i = 0; i < kpis.size(); i++) {
            ...
            json.append("        \"status\": ").append(quote(kpi.status().name())).append("\n");
            json.append("      }");
    ...
    private void appendBoundedContexts(StringBuilder json, List<BoundedContextInventory> contexts) {
        ...
        for (int i = 0; i < contexts.size(); i++) {
            ...
            json.append("          \"valueObjects\": ").append(bc.valueObjects()).append(",\n");
            json.append("          \"domainEvents\": ").append(bc.domainEvents()).append("\n");
            json.append("        }");
    ...
    private void appendAggregates(StringBuilder json, List<AggregateComponent> aggregates) {
        ...
        for (int i = 0; i < aggregates.size(); i++) {
            ...
            json.append("          \"package\": ").append(quote(agg.packageName())).append(",\n");
            json.append("          \"fields\": ").append(agg.fields()).append(",\n");
            json.append("          \"references\": ").append(toJsonArray(agg.references())).append(",\n");
            json.append("          \"usesPorts\": ").append(toJsonArray(agg.usesPorts())).append("\n");
            json.append("        }");
    ...
    private void appendValueObjects(StringBuilder json, List<ValueObjectComponent> vos) {
        ...
        for (int i = 0; i < vos.size(); i++) {
            ...
            json.append("          \"package\": ").append(quote(vo.packageName())).append("\n");
            json.append("        }");
    ...
    private void appendIdentifiers(StringBuilder json, List<IdentifierComponent> ids) {
        ...
        for (int i = 0; i < ids.size(); i++) {
            ...
            json.append("          \"package\": ").append(quote(id.packageName())).append(",\n");
            json.append("          \"wrappedType\": ").append(quote(id.wrappedType())).append("\n");
            json.append("        }");
    ...
    private void appendPorts(StringBuilder json, List<PortComponent> ports) {
        ...
        for (int i = 0; i < ports.size(); i++) {
            ...
            json.append("          \"package\": ").append(quote(port.packageName())).append(",\n");
    ...
    private void appendAdapters(StringBuilder json, List<AdapterComponent> adapters) {
        ...
        for (int i = 0; i < adapters.size(); i++) {
            ...
            json.append("          \"package\": ").append(quote(adapter.packageName())).append(",\n");
            json.append("          \"implementsPort\": ").append(quote(adapter.implementsPort())).append(",\n");
            json.append("          \"type\": ").append(quote(adapter.type().name())).append("\n");
            json.append("        }");
    ...
    private void appendIssueGroups(StringBuilder json, List<IssueGroup> groups) {
        ...
        for (int i = 0; i < groups.size(); i++) {
            ...
            json.append("        \"description\": ").append(quote(group.description())).append(",\n");
    ...
    private void appendIssueEntries(StringBuilder json, List<IssueEntry> entries) {
        ...
        for (int i = 0; i < entries.size(); i++) {
            ...
            json.append("            \"constraintId\": ").append(quote(entry.constraintId())).append(",\n");
            json.append("            \"severity\": ").append(quote(entry.severity().name())).append(",\n");
            json.append("            \"title\": ").append(quote(entry.title())).append(",\n");
            json.append("            \"message\": ").append(quote(entry.message())).append(",\n");
            json.append("            \"location\": {\n");
            json.append("              \"type\": ").append(quote(entry.location().type())).append(",\n");
            json.append("              \"file\": ").append(quote(entry.location().file())).append(",\n");
            json.append("              \"line\": ").append(entry.location().lineOpt().map(String::valueOf).orElse("null")).append("\n");
            json.append("            },\n");
            json.append("            \"impact\": ").append(quote(entry.impact())).append(",\n");
            json.append("            \"suggestion\": {\n");
            Suggestion s = entry.suggestion();
            json.append("              \"action\": ").append(quote(s.action())).append(",\n");
            json.append("              \"steps\": ").append(toJsonArray(s.steps())).append(",\n");
            json.append("              \"codeExample\": ").append(quote(s.codeExampleOpt().orElse(null))).append(",\n");
            json.append("              \"effort\": ").append(quote(s.effortOpt().orElse(null))).append("\n");
            json.append("            }\n");
            json.append("          }");
    ...
    private void appendRemediation(StringBuilder json, RemediationPlan plan) {
        ...
        json.append("      \"days\": ").append(String.format(Locale.US, "%.1f", effort.days())).append(",\n");
        if (effort.costOpt().isPresent()) {
            CostEstimate cost = effort.costOpt().get();
            json.append("      \"cost\": {\n");
            json.append("        \"amount\": ").append(String.format(Locale.US, "%.0f", cost.amount())).append(",\n");
            json.append("        \"currency\": ").append(quote(cost.currency())).append(",\n");
            json.append("        \"dailyRate\": ").append(String.format(Locale.US, "%.0f", cost.dailyRate())).append("\n");
            json.append("      }\n");
        } else {
    ...
    private void appendRemediationActions(StringBuilder json, List<RemediationAction> actions) {
        ...
        for (int i = 0; i < actions.size(); i++) {
            ...
            json.append("        \"severity\": ").append(quote(action.severity().name())).append(",\n");
            json.append("        \"title\": ").append(quote(action.title())).append(",\n");
            json.append("        \"description\": ").append(quote(action.description())).append(",\n");
            json.append("        \"effort\": {\n");
            json.append("          \"days\": ").append(String.format(Locale.US, "%.1f", action.effort().days())).append(",\n");
            json.append("          \"description\": ").append(quote(action.effort().description())).append("\n");
            json.append("        },\n");
            json.append("        \"impact\": ").append(quote(action.impact())).append(",\n");
            json.append("        \"affectedTypes\": ").append(toJsonArray(action.affectedTypes())).append(",\n");
            json.append("        \"relatedIssues\": ").append(toJsonArray(action.relatedIssues())).append("\n");
            json.append("      }");
    ...
    private void appendScoreBreakdown(StringBuilder json, ScoreBreakdown breakdown) {
        json.append("{\n");
        json.append("      \"dddCompliance\": ").append(formatDimension(breakdown.dddCompliance())).append(",\n");
        json.append("      \"hexagonalCompliance\": ").append(formatDimension(breakdown.hexagonalCompliance())).append(",\n");
        json.append("      \"dependencyQuality\": ").append(formatDimension(breakdown.dependencyQuality())).append(",\n");
        json.append("      \"couplingMetrics\": ").append(formatDimension(breakdown.couplingMetrics())).append(",\n");
        json.append("      \"cohesionQuality\": ").append(formatDimension(breakdown.cohesionQuality())).append("\n");
        json.append("    }");
    }
    ...
    private void appendMetrics(StringBuilder json, List<MetricEntry> metrics) {
        ...
        for (int i = 0; i < metrics.size(); i++) {
            ...
            json.append("        \"value\": ").append(String.format(Locale.US, "%.2f", m.value())).append(",\n");
    ...
    private void appendConstraints(StringBuilder json, List<ConstraintResult> constraints) {
        ...
        for (int i = 0; i < constraints.size(); i++) {
            ...
            json.append("      {\"id\": ").append(quote(c.id()))
                    .append(", \"violations\": ").append(c.violations()).append("}");
    ...
    private void appendPackageMetrics(StringBuilder json, List<PackageMetric> packages) {
        ...
        for (int i = 0; i < packages.size(); i++) {
            ...
            json.append("        \"instability\": ").append(String.format(Locale.US, "%.2f", pm.instability())).append(",\n");
            json.append("        \"abstractness\": ").append(String.format(Locale.US, "%.2f", pm.abstractness())).append(",\n");
            json.append("        \"distance\": ").append(String.format(Locale.US, "%.2f", pm.distance())).append(",\n");
