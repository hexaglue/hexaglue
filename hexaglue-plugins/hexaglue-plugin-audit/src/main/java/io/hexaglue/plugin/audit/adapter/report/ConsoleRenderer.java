...
public class ConsoleRenderer implements ReportRenderer {
    ...
    public String render(ReportData data, DiagramSet diagrams) {
        ...
        out.append(bold("-".repeat(70))).append("\n");
        out.append("  Generated by HexaGlue ")
                .append(data.metadata().hexaglueVersion())
                .append("\n");
        out.append(bold("-".repeat(70))).append("\n\n");
    ...
    private void renderVerdict(StringBuilder out, Verdict verdict) {
        ...
        // Score with color
        String scoreColor = verdict.score() >= 80 ? ANSI_GREEN : verdict.score() >= 60 ? ANSI_YELLOW : ANSI_RED;
        ...
        if (!verdict.kpis().isEmpty()) {
            ...
            for (KPI kpi : verdict.kpis()) {
                ...
                out.append("    ")
                        ...
                        .append(" (threshold: ")
                        .append(String.format(Locale.ROOT, "%.0f", kpi.threshold()))
                        .append("%)\n");
            }
            out.append("\n");
        }
    ...
    private void renderIssuesSummary(StringBuilder out, IssuesSummary issues) {
        ...
        if (counts.blockers() > 0) {
            out.append("    ")
                    .append(color("\u2588", ANSI_PURPLE))
                    .append(" BLOCKER:  ")
                    .append(counts.blockers())
                    .append("\n");
        }
        if (counts.criticals() > 0) {
            out.append("    ")
                    .append(color("\u2588", ANSI_RED))
                    .append(" CRITICAL: ")
                    .append(counts.criticals())
                    .append("\n");
        }
        if (counts.majors() > 0) {
            out.append("    ")
                    .append(color("\u2588", ANSI_YELLOW))
                    .append(" MAJOR:    ")
                    .append(counts.majors())
                    .append("\n");
        }
        if (counts.minors() > 0) {
            out.append("    ")
                    .append(color("\u2588", ANSI_CYAN))
                    .append(" MINOR:    ")
                    .append(counts.minors())
                    .append("\n");
        }
        if (counts.infos() > 0) {
            out.append("    ")
                    .append(color("\u2588", ANSI_GREEN))
                    .append(" INFO:     ")
                    .append(counts.infos())
                    .append("\n");
        }
    ...
    private void renderRemediationSummary(StringBuilder out, RemediationPlan plan) {
        ...
        out.append("  Total Effort: ")
                .append(String.format(Locale.ROOT, "%.1f", effort.days()))
                .append(" days");
        effort.costOpt().ifPresent(cost -> out.append(" (~")
        ...
        for (RemediationAction action : plan.actions()) {
            ...
            out.append(" (")
                    .append(String.format(Locale.ROOT, "%.1f", action.effort().days()))
                    .append("d)\n");
            shown++;
        }
