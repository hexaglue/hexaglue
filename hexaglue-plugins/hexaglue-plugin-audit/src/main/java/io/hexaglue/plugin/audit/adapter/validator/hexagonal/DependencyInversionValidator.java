...
public class DependencyInversionValidator implements ConstraintValidator {
    ...
    public List<Violation> validate(Codebase codebase, ArchitectureQuery query) {
        ...
        // Check application layer dependencies to infrastructure
        for (CodeUnit unit : codebase.unitsInLayer(LayerClassification.APPLICATION)) {
            violations.addAll(validateDependencyInversion(codebase, unit));
        }
     ...
     * @param unit the application layer unit to check
     * @return list of violations found
     */
    private List<Violation> validateDependencyInversion(Codebase codebase, CodeUnit unit) {
        List<Violation> violations = new ArrayList<>();
        ...
        for (String depName : deps) {
            ...
            codebase.units().stream()
                    ...
                    .ifPresent(infraUnit -> {
                        ...
                        if (infraUnit.kind() != CodeUnitKind.INTERFACE) {
                            violations.add(Violation.builder(CONSTRAINT_ID)
                                    ...
                                    .message(("Application type '%s' depends on concrete Infrastructure type '%s' "
                                            ...
                                            .formatted(unit.simpleName(), infraUnit.simpleName()))
