...
/**
 * Classifies types as port interfaces (DRIVING_PORT, DRIVEN_PORT, APPLICATION_SERVICE).
 *
 * <p>Only interface types can be classified as ports. Classes, records, and enums
 * are not considered ports and will return UNCLASSIFIED.</p>
 *
 ...
 *
 * if (trace.classifiedAs().isPort()) {
 *     System.out.println("Port type: " + trace.classifiedAs());
 * }
 * }</pre>
 *
 * @since 4.0.0
...
public final class PortClassifier {
    ...
    /**
     * Creates a trace for non-port types (classes, enums, records).
     */
    private ClassificationTrace createNotAPortTrace() {
        AppliedCriterion notApplicable = new AppliedCriterion(
                "not-an-interface",
                0,
                false,
                ElementKind.UNCLASSIFIED,
                "Only interfaces can be classified as ports",
                List.of());
